...
import { setDbUser, setNameForUser } from "./data/user";
...
export const { handlers, signIn, signOut, auth } = NextAuth({
    ...
    callbacks: {
        ...
        authorized({ auth, request: { nextUrl } }) {
            ...
            const isProtected = protectedRoutes.reduce(
                (acc: boolean, route: string) => {
        ...
        async jwt({ token, account, profile }) {
            ...
            if (account) {
                if (profile) {
                    ...
                    const metadata: any = await getUserMetadata(
                        profile.sub ?? ""
                    );
        ...
        session({ session, token }) {
            ...
            return {
                ...
                auth0Picture: token.auth0Picture,
                auth0Username: token.auth0Username,
            };
        },
    },
});
